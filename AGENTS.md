...
├── justfile                # Build and run commands (task runner: https://github.com/casey/just)
└── AGENTS.md              # This file
```
...
just build
just run-core      # Core services only
just run-core-workers  # Core + workers

# Build only
just build         # Build all Docker images

# Stop and cleanup
just stop          # Stop running services
just remove        # Stop and remove containers/volumes
just clean-all     # Prune Docker system

# Check status
just check         # Check service health
just show-images   # Show built Docker images
```
...
**Important**: Never run `just build` from this directory. Instead, always rebuild individual images using the rebuild script:
...
### Option A: Full build
```bash
just build           # Builds all images (calls export-requirements.sh automatically)
just build-no-cache  # Full rebuild without cache
```
